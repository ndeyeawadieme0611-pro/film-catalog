...
api.interceptors.response.use(
  ...
  (err) => {
    if (import.meta.env.DEV) {
      console.warn(
        "[API] Erreur réseau — fallback données locales",
        err.message,
      );
    }
  ...
  try {
    ...
    return {
      ...
      avgRating: movies.length
        ? (
            movies.reduce((acc, m) => acc + (m.vote_average || 0), 0) /
            movies.length
          ).toFixed(1)
        : 0,
      topGenre: "N/A",
...
export async function fetchYears() {
  ...
  return ["Toutes", ...(data.years || []).map(String)];
}
...
export async function registerUser(email, password, fname, lname) {
